...
const UpscaleImage: React.FC = () => {
  ...
  const applyAISharpen = async () => {
    if (!imageFile) return;
    setIsLoading(true);
    setSharpenedUrl(null);
    try {
        const resultUrl = await upscaleImageWithAI(imageFile);
        setSharpenedUrl(resultUrl);
    } catch (error: any) {
        alert(error.message || "An unknown error occurred with AI Image Sharpening.");
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const handleDownload = () => {
    if (!sharpenedUrl || !imageFile) return;
    const filename = `sharpened_${imageFile.name}`;
    downloadImage(sharpenedUrl, filename);
  };
  ...
  const AIInfoBox = () => (
    <div className="p-4 bg-gray-900/70 text-gray-400 border border-gray-700 rounded-lg text-sm">
        <p><span className="font-semibold text-teal-400">Note:</span> This tool uses an AI model to enhance image resolution and details, which includes sharpening.</p>
        <p className="mt-2">Images larger than 1000x1000 pixels may be downscaled to meet API requirements.</p>
    </div>
  );
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 space-y-6">
            <h3 className="text-lg font-semibold text-white">AI Image Upscaler</h3>
            <AIInfoBox />
              <div className="flex flex-col gap-4 pt-4 border-t border-gray-700">
                <Button icon={<UpscaleIcon />} onClick={applyAISharpen} isLoading={isLoading} disabled={!imageFile}>Upscale with AI</Button>
                <Button icon={<ArrowDownTrayIcon />} onClick={handleDownload} variant="secondary" disabled={!sharpenedUrl}>
                    Download
                </Button>
            </div>
        </div>
            ...
            <>
              <div className="flex justify-center mb-4">
                <div className="inline-flex rounded-md shadow-sm bg-gray-700/50 p-1">
                  ...
                  <button onClick={() => setViewMode('side-by-side')} disabled={!sharpenedUrl} className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'side-by-side' ? 'bg-teal-500 text-white' : 'text-gray-300 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed'}`}>
                    Compare
                  </button>
                </div>
              </div>
              ...
              {sharpenedUrl && !isLoading && (
                  <div className="mt-4 p-3 bg-teal-500/10 border border-teal-500/20 rounded-lg text-center">
                      <p className="font-medium text-sm text-teal-300">Upscaling applied successfully!</p>
                  </div>
              )}
            </>
          )}
